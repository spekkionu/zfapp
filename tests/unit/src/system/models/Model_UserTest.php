...
class Model_UserTest extends Test_DbTestCase
{
  ...
  public function testEncryptPassword() {
    $raw = 'password';
    $encrypted = Model_User::encryptPassword($raw);
    $this->assertNotEquals($raw, $encrypted);
  }

  /**
   * tests Model_User::encryptToken
   */
  public function testEncryptToken() {
    $raw = md5(uniqid(time(), true));
    $encrypted = Model_User::encryptToken($raw);
    $this->assertNotEquals($raw, $encrypted);
  }

  /**
   * tests Model_User::encryptPin
   */
  public function testEncryptPin() {
    $raw = '123';
    $encrypted = Model_User::encryptPin($raw);
    $this->assertNotEquals($raw, $encrypted);
  }
