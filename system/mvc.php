...
$application = new Zend_Application(APPLICATION_ENV, SYSTEM . '/configs/application.ini');
...
if (APPLICATION_ENV != 'testing') {
    ...
    if ($config['debug']['xhprof']['active']) {
        ...
        $runid = $xhprof_runs->save_run($xhprof_data, $config['debug']['xhprof']['key']);
        $url = $config['debug']['xhprof']['base_url'] . "?run={$runid}&source={$config['debug']['xhprof']['key']}";
    }
